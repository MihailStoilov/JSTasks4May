...
createRobot.addEventListener("click", (event) => {
  if (robotName.value === "" || robotType.value === "" || robotColor.value === "") {
    ...
    function createDynamicRobot() {
      ...
      newSection.classList.add("factory-section");
      newSection.setAttribute("id", `slide-${robots.length + 1}`);

      const createSection = document.querySelector('section');
      createSection.insertAdjacentElement("beforebegin", newSection);
    ...
    robotPhrase.value = "";
    console.log(robotType.options[robotType.selectedIndex].text);
  }
